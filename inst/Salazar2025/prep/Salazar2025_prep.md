...
Data origin reference: Several, looked up by Carlos Salazar Martín

Primary data compiler: Carlos Salazar Martín
...
```
    ...
    ## New names:
    ## rename: renamed 3 variables (Taxon, UTM10x10, Notas)
    ## • `` -> `...2`
    ## • `` -> `...3`
...
```
    ...
    ## mutate: new variable 'Taxon_ok' (character) with 72 unique values and 0% NA
    ## mutate: new variable 'References' (character) with one unique value and 0% NA
    ## mutate: changed 2 values (1%) of 'Taxon_ok' (0 new NAs)
    ## select: dropped 2 variables (Notas, Taxon_ok)
...
```
    ...
    ## There are 27 non-valid taxa Erasing them.

    ## mutate: no changes
    ## There are 2 non-valid cells. Erasing them.
...
```
    ...
    ## filter: removed 195 rows (50%), 197 rows remaining
