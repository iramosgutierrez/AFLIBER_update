...
Data origin reference: Several papers

Primary data compiler: Manuel Marquerie
...
``` r
citationkey <- "Marquerie2021"
# dt_raw <- readr::read_csv(here(paste0("inst/",citationkey,"/raw/XXXXXX.csv")))
dt_raw <- readxl::read_excel(here(paste0("inst/",citationkey,"/raw/UTMS_Jul_biblio20-21_ManuMarquerie_RevIRG.xlsx")))
```
...
```
    ...
    ## rename: renamed 2 variables (Taxon, UTM10x10)
    ## mutate: new variable 'UTM1x1' (logical) with one unique value and 100% NA
    ##         new variable 'References' (character) with 78 unique values and 0% NA
...
```
    ...
    ## There are 388 non-valid taxa Erasing them.

    ## mutate: no changes
    ## There are 41 non-valid cells. Erasing them.
...
```
    ...
    ## filter: removed 1,263 rows (60%), 826 rows remaining
