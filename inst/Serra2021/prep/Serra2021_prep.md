...
Data origin reference: Lluís Serra Laliga. 2021. El patrimoni vegetal de
Monòver. Ajuntament de Monòver. 590 pp. ISBN 978-84-09-36962-I

Primary data compiler: Lluís Serra Laliga
...
``` r
citationkey <- "Serra2021"
# dt_raw <- readr::read_csv(here(paste0("inst/",citationkey,"/raw/XXXXXX.csv")))
dt_raw <- readxl::read_excel(here(paste0("inst/",citationkey,"/raw/flora monover_SERRA_2021.xlsx")))
```
...
dt_modif <- dt_raw |> 
  ...
  mutate(References = "Serra Laliga, L. 2021. El patrimoni vegetal de Monòver. ISBN 978-84-09-36962-I") |> 
...
```
    ...
    ## mutate: no changes
    ## mutate: no changes
    ## filter: removed 74 rows (<1%), 19,982 rows remaining
    ## mutate: changed 19,982 values (100%) of 'UTM1x1' (0 new NAs)
    ## mutate: changed 19,982 values (100%) of 'UTM10x10' (0 new NAs)
    ## mutate: changed 5,897 values (30%) of 'Taxon' (0 new NAs)
    ## mutate: new variable 'References' (character) with one unique value and 0% NA
    ## distinct: removed 548 rows (3%), 19,434 rows remaining
...
```
    ...
    ## There are 164 non-valid taxa Erasing them.
...
```
    ...
    ## filter: removed 17,169 rows (88%), 2,265 rows remaining
