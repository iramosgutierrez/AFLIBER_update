...
Data origin reference: <https://doi.org/10.15470/wiadvo> 4.654
occurrences of letters B & C to upload to GBIF

Primary data compiler: Carlos Salazar Mendías
...
```
    ...
    ## mutate: changed 4,643 values (>99%) of 'Taxon' (0 new NAs)
    ## mutate: changed one value (<1%) of 'Coordenadas' (0 new NAs)
    ## mutate: new variable 'UTM1x1' (character) with 1,552 unique values and 1% NA
    ## mutate: new variable 'UTM10x10' (character) with 288 unique values and <1% NA
    ## filter: removed 10 rows (<1%), 4,644 rows remaining
    ## filter: removed 3 rows (<1%), 4,641 rows remaining
    ## mutate: new variable 'References' (character) with one unique value and 0% NA
    ## select: dropped 21 variables (Numero registro JAEN, Colector, Fecha, Hábitat, País, …)
...
```
    ...
    ## There are 223 non-valid taxa Erasing them.
...
```
    ...
    ## filter: removed 3,121 rows (67%), 1,520 rows remaining
