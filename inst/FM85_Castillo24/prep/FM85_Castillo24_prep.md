...
Data origin reference: Flora Montiberica 85 (2023)
<http://www.floramontiberica.org/FM/Flora_Montiberica85.pdf>

Primary data compiler: Elvira Castillo
...
``` r
citationkey <- "FM85_Castillo24"
# dt_raw <- readr::read_csv(here(paste0("inst/",citationkey,"/raw/XXXXXX.csv")))
dt_raw <- readxl::read_excel(here(paste0("inst/",citationkey,"/raw/FLORA MONTIBÉRICA 85.xlsx")), sheet = 2)
```
...
#modify variables
dt_modif <- dt_raw |> 
  mutate(Taxon = ifelse(!is.na(Especie_AFLIBER), Especie_AFLIBER, Especie_FM)) |> 
...
```
    ...
    ## mutate: new variable 'Taxon' (character) with 154 unique values and 0% NA
    ## rename: renamed 3 variables (UTM1x1, UTM10x10, References)
    ## select: dropped 6 variables (Especie_FM, Especie_AFLIBER, UTM_original, Sp. Nov., Observaciones, …)
...
```
    ...
    ## There are 42 non-valid taxa Erasing them.

    ## mutate: no changes
    ## There are 14 non-valid cells. Erasing them.
...
```
    ...
    ## filter: removed 334 rows (73%), 121 rows remaining
