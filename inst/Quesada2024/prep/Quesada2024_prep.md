...
Data origin reference: Herbario Universidad de Granada (GDA) data for
GBIF <https://doi.org/10.15468/pyzji3>

Primary data compiler: Carmen Quesada, Juan Carlos Moreno Saiz

Other data compilers: Juan Lorite
...
```
    ...
    ## mutate: changed 630 values (19%) of 'Taxon' (0 new NAs)
    ## mutate: new variable 'References' (character) with one unique value and 0% NA
    ## distinct: removed 200 rows (6%), 3,090 rows remaining
...
```
    ...
    ## There are 505 non-valid taxa Erasing them.

    ## mutate: no changes
    ## There are 328 non-valid cells. Erasing them.
...
```
    ...
    ## filter: removed 2,030 rows (66%), 1,060 rows remaining
