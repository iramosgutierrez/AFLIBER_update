...
Primary data compiler: Estrella Alfaro-Saiz
...
```
    ...
    ## filter: removed 39 rows (1%), 3,729 rows remaining
    ## filter: removed 300 rows (8%), 3,429 rows remaining
    ## filter: removed 457 rows (13%), 2,972 rows remaining
    ## mutate: new variable 'Taxon' (character) with 1,844 unique values and 0% NA
    ## mutate: changed 1,333 values (45%) of 'UTM' (0 new NAs)
    ## filter: removed 734 rows (25%), 2,238 rows remaining
    ## mutate: new variable 'nchar' (integer) with 15 unique values and 0% NA
    ## mutate: new variable 'UTM10x10' (character) with 680 unique values and 28% NA
    ## mutate: new variable 'UTM1x1' (character) with 886 unique values and 39% NA
    ## mutate: new variable 'References' (character) with one unique value and 0% NA
    ## select: dropped 8 variables (GENERO, HIB, ESPECIE, INFRANK, INFRA, …)
...
```
    ...
    ## There are 536 non-valid taxa Erasing them.

    ## mutate: no changes
    ## There are 531 non-valid cells. Erasing them.
...
```
    ...
    ## filter: removed 983 rows (43%), 1,305 rows remaining
