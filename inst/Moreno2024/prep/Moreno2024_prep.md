...
Data origin reference: Several origins (FloraMontiberica, Acta Botanica
Malacitana, TFGs…).

Primary data compiler: Juan Carlos Moreno Saiz
...
```
    ...
    ## mutate: new variable 'References' (character) with 13 unique values and 0% NA
    ## mutate: changed 6 values (2%) of 'Taxon' (0 new NAs)
    ## rename: renamed 2 variables (UTM1x1, UTM10x10)
    ## select: dropped 2 variables (Referencia (url/doi), Observaciones)
...
```
    ...
    ## mutate: no changes
    ## select: dropped 5 variables (Referencia (url/doi), Observaciones, ...6, ...7, ...8)
...
```
    ...
    ## mutate: changed 157 values (10%) of 'Taxon' (0 new NAs)
...
```
    ...
    ## There are 215 non-valid taxa Erasing them.
...
```
    ...
    ## filter: removed 1,386 rows (77%), 409 rows remaining
