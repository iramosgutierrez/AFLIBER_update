...
Data origin reference: Flora Montiberica 86 (2023)
<http://www.floramontiberica.org/FM/Flora_Montiberica86.pdf>
...
``` r
citationkey <- "FM86_Castillo24"
# dt_raw <- readr::read_csv(here(paste0("inst/",citationkey,"/raw/XXXXXX.csv")))
dt_raw <- readxl::read_excel(here(paste0("inst/",citationkey,"/raw/FLORA MONTIBÉRICA 86.xlsx")), sheet= 2)
```
...
```
    ...
    ## mutate: new variable 'Taxon' (character) with 179 unique values and 3% NA
    ## rename: renamed 3 variables (UTM1x1, UTM10x10, References)
    ## select: dropped 6 variables (Especie_FM, Especie_AFLIBER, ¿UTM 0,5x0,5?, Sp. Nov., Observaciones, …)
...
```
    ...
    ## There are 55 non-valid taxa Erasing them.

    ## mutate: no changes
    ## There are 12 non-valid cells. Erasing them.
...
```
    ...
    ## filter: removed 280 rows (57%), 211 rows remaining
